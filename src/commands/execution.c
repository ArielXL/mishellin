            ...
            if (command->has_output_app) {
                for (int i = 0; command->output_files_app[i]; ++i) {
                    ...
                    if ((fdout = open(command->output_files_app[i], O_WRONLY | O_APPEND)) < 0) {
                        fdout = open(command->output_files_app[i], O_WRONLY | O_APPEND | O_CREAT, 0644);
                    }
...
int execute(Command_t *command, int fdin, int its_last) {
    ...
    if (pid == 0) {
        ...
        if (!its_last || command->has_output || command->has_output_app) {
            dup2(fdout, 1);
            // printf("Si tiene redirccion %s append: %d trunc: %d last: %d\n", command.lex, command.has_output_app, command.has_output, its_last);
        }
